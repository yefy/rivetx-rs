...
pub async fn test_create_table(rivetx_sql: &RivetxSql) -> anyhow::Result<()> {
    ...
    test_verify_test_data_table_structure(rivetx_sql).await?;
    test_verify_test_key_table_structure(rivetx_sql).await?;
...
pub async fn test_create_table_with_timeout(rivetx_sql: &RivetxSql) -> anyhow::Result<()> {
    ...
    create_table::<Testkey>(rivetx_sql, &"test_key".into(), Duration::from_secs(2)).await?;
