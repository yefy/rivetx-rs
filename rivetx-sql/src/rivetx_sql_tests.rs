...
pub async fn rivetx_sql_tests() -> anyhow::Result<()> {
    ...
    test_create(&rivetx_sql).await?;

    log::info!("test_insert");
    test_insert(&rivetx_sql).await?;
    test_insert2(&rivetx_sql).await?;

    log::info!("test_select_all");
    test_select(&rivetx_sql).await?;

    log::info!("test_delete");
    test_delete(&rivetx_sql).await?;

    log::info!("test_update");
    test_update(&rivetx_sql).await?;
    Ok(())
}
