        ...
        if let Fields::Named(fields) = data.fields {
            for (i, field) in fields.named.into_iter().enumerate() {
                ...
                let sql_type = match type_str.as_str() {
                    "u64" | "u128" | "usize" => "BIGINT UNSIGNED".to_string(),
                    "u32" | "u16" | "u8" => "INT UNSIGNED".to_string(),
                    "i64" | "i128" | "isize" => "BIGINT".to_string(),
                    "i32" | "i16" | "i8" => "INT".to_string(),
                    "String" | "&str" => {
    ...
    let expanded = quote! {
        ...
        impl FromSqlRow for #name {
            fn get_struct_meta() -> crate::StructMeta {
                ...
                type RString = rivetx_core_rs::rivetx_string::RivetxString;
