...
fn test_arc_atomic_u32_try_from_value_ok() {
    ...
    assert_eq!(ArcAtomicU32::try_from(Value::Float(3.0)).unwrap().as_u32(), 3);
    assert_eq!(ArcAtomicU32::try_from(Value::Double(4.0)).unwrap().as_u32(), 4);
    assert_eq!(ArcAtomicU32::try_from(Value::Bytes(b"5".to_vec())).unwrap().as_u32(), 5);
}
