...
fn test_arc_atomic_i64_try_from_value_ok() {
    ...
    assert_eq!(ArcAtomicI64::try_from(Value::Float(3.0)).unwrap().as_i64(), 3);
    assert_eq!(ArcAtomicI64::try_from(Value::Double(4.0)).unwrap().as_i64(), 4);
    assert_eq!(ArcAtomicI64::try_from(Value::Bytes(b"5".to_vec())).unwrap().as_i64(), 5);
}
