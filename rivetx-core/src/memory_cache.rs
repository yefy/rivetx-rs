...
impl<K, V> Expiry<K, ValueWithTtl<V>> for DynamicExpiry {
    ...
    fn expire_after_create(
        ...
        value
            .expires_at
            .map(|expires_at| {
                expires_at.saturating_duration_since(created_at)
            })
    }
    ...
    fn expire_after_update(
        ...
        value
            .expires_at
            .map(|expires_at| {
                expires_at.saturating_duration_since(updated_at)
            })
    }
}
...
impl MemoryCache {
    ...
    pub fn incrbyfloat(
        ...
        if !new_value.is_finite() {
            return Err(
                "increment would produce a non-finite number"
                    .to_string(),
            );
        }
    ...
    fn get_or_create_table(
        ...
        let value = self.table_cache.get_with(
            table_name,
            || ValueWithTtl {
                value: Arc::new(
                    RwLock::new(
    ...
    pub fn hget(
        ...
        let value =
            self.table_cache.get(table)?;

        let map =
            value.value.read().unwrap();
