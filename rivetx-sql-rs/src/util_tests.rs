...
pub async fn test_data_query_all(rivetx_sql: &RivetxSql) -> anyhow::Result<Vec<TestData>> {
    let mut conn = rivetx_sql.conn().await?;
    let query = "SELECT id, index_col, key_col, name_id, name_index FROM test_data ORDER BY index_col, key_col";

    let result = conn
        .query_map(query, |(id, index, key, name_id, name_index)| TestData {
            id,
            index,
            key,
            name_id,
...
pub async fn test_data_query_all_no_id(rivetx_sql: &RivetxSql) -> anyhow::Result<Vec<TestData>> {
    let mut conn = rivetx_sql.conn().await?;
    let query =
        "SELECT index_col, key_col, name_id, name_index FROM test_data ORDER BY index_col, key_col";

    let result = conn
        .query_map(query, |(index, key, name_id, name_index)| {
            TestData {
                id: 0, // 模拟 Scan 忽略 Id
                index,
                key,
...
pub async fn test_data_query_all_by_id(
    ...
    let query = format!(
        "SELECT id, index_col, key_col, name_id, name_index FROM test_data ORDER BY id {} LIMIT {}",
        order, limit
    );
